...
describe("heartbeat comment wake batching", () => {
  ...
  it("auto-completes conference room turns after the agent replies", async () => {
    const gateway = await createControlledGatewayServer();
    const companyId = randomUUID();
    const agentId = randomUUID();
    const issueId = randomUUID();
    ...
    try {
      ...
      expect(completedIssue?.checkoutRunId).toBeNull();
      expect(completedIssue?.executionRunId).toBeNull();
      expect(completedIssue?.completedAt).toBeInstanceOf(Date);
